...
describe Fastlane::Actions::FirebaseTestLabAndroidAction do
  ...
  describe '#run' do
    ...
    it 'calls deps appropriately' do
      allow(FirebaseTestLabIntegration::Helper::IntegrationHelper).to receive(:new).with(:android).and_return(integration_mock)
      allow(integration_mock).to receive(:run).with(params).and_return(nil)
      expect(FastlaneCore::UI).to receive(:message).with("Starting Firebase Test Lab Integration for Android.")
      expect(FastlaneCore::UI).to receive(:message).with("Completed Firebase Test Lab Integration for Android.")
  ...
  describe '#description' do
    it 'returns correct data' do
      expect(described_class.description).to eq("Run Android integration tests on Firebase Test Lab")
    end
  end
